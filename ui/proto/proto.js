...
function Meter(percentage, last, val, label, canvas) {

    if (last) {

        var newto = 140 + ((350 - 140) * (percentage / 100));
        last.ctx.lineWidth = 17;
        last.ctx.beginPath();
        last.ctx.arc(200, 200, 180, degToRad(last.to), degToRad(newto));
        last.ctx.stroke();

        last.ctx.font = "100px Helvetica";
        last.ctx.fillStyle = '#28d1fa';
        last.ctx.fillText(val, 75, 220);

        return { from: to, to:newto, canvas: last.canvas, ctx: last.ctx };
    }
    else {
        ...
        return { from: from, to: to, canvas: canvas, ctx: ctx };
    }
